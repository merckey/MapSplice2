...
public:
	...
	string getRefname(uint32_t index)
	{
		return (*_refnames)[index];
	}
...
public:
	...
	virtual void bufferHit(const Hit& h, int stratum) {
		...
		//cout << "buffer hit for read: " << h.patName <<" " <<_bufferedHits.size() << endl;
	}
	...
	bool convert_full_read(vector<Bwtmap_Info> & mapsplice_vec, bool paired, int _pair_no, int start_segment_no, int end_segment_no, bool debug)
	{
		if(debug)
		{
			if(_bufferedHits.size() > 0)
				cout << "full read mapping " << _bufferedHits[0].patName << "\t" << _bufferedHits.size() << "\t" << _max << endl;
			else
				cout << "no full read mapped" << endl;
		}
		if(_bufferedHits.size() == 0 || _bufferedHits.size() >= _max ) 
		{
				return false;
		}
		for(size_t i = 0; i < _bufferedHits.size(); i++)
		{
			Bwtmap_Info new_info;
			new_info.pair_no = _pair_no;
			if(_bufferedHits[i].fw)
				new_info.strand = "+";
			else
				new_info.strand = "-";
			size_t len = length(_bufferedHits[i].patSeq);
			string all_chrom = _sink.getRefname(_bufferedHits[i].h.first);
			char clean_chrom[1000];
			sscanf(all_chrom.c_str(), "%s\t", clean_chrom);
			new_info.chrom = clean_chrom;
			new_info.start = _bufferedHits[i].h.second;
			new_info.end = new_info.start + len - 1;
			char tmp_readname[1000];
			assign(tmp_readname, _bufferedHits[i].patName);
			new_info.read_id = tmp_readname;
			new_info.start_seg_no = (new_info.strand == "+" ? start_segment_no : end_segment_no);
			new_info.end_seg_no = (new_info.strand == "+" ? end_segment_no : start_segment_no);
			mapsplice_vec.push_back(new_info);
		}
		return true;
	}
	
	bool convert_full_read(vector<Bwtmap_Info> & mapsplice_vec, bool paired, int _pair_no, int start_segment_no, int end_segment_no, string chrom, int adjust_base, bool debug)
	{
		if(debug)
		{
			if(_bufferedHits.size() > 0)
		...
		if(_bufferedHits.size() >= _max || _bufferedHits.size() == 0) 
			return false;
		for(size_t i = 0; i < _bufferedHits.size(); i++)
		{
			Bwtmap_Info new_info;
			...
			new_info.start = _bufferedHits[i].h.second + adjust_base;
			new_info.end = new_info.start + len - 1;
			char tmp_readname[1000];
			assign(tmp_readname, _bufferedHits[i].patName);
			new_info.read_id = tmp_readname;
	...
	bool convert_segment(vector<Bwtmap_Info>& mapsplice_vec, bool paired, int _pair_no, int segment_no, bool debug)
	{
		if(debug)
		{
			if(_bufferedHits.size() > 0)
				cout << "segmnent mapping " << _bufferedHits[0].patName << "\t" << _bufferedHits.size() << "\t" << _max << endl;
			else
				cout << "no segmnent mapped" << endl;
		}
		if(_bufferedHits.size() >= _max) 
		{
				return false;
		}
		for(size_t i = 0; i < _bufferedHits.size(); i++)
		{
			...
			new_info.chrom = clean_chrom;
			new_info.start = _bufferedHits[i].h.second;
			char tmp_readname[1000], tmp_readid[1000];
			assign(tmp_readname, _bufferedHits[i].patName);
			sscanf(tmp_readname, "%[^~]%*[~]%d", tmp_readid, &(new_info.start_seg_no));
			new_info.read_id = tmp_readid;
			new_info.end_seg_no = new_info.start_seg_no;
			new_info.end = new_info.start + length(_bufferedHits[i].patSeq) - 1;
			size_t len = length(_bufferedHits[i].patSeq);
			for (int j = 0; j < (int)len; ++ j) 
			{
				if(_bufferedHits[i].mms.test(j)) 
						new_info.num_mis ++;
			}
			if(debug)
				cout << "seg " << new_info.start_seg_no << "\t" << new_info.chrom << "\t" <<new_info.strand << "\t" << new_info.start << "\t" << new_info.end << "\t" << new_info.num_mis << endl; 
			mapsplice_vec.push_back(new_info);
		}
		return true;
	}

	void convert_anchor(vector<Bwtmap_Info> & mapsplice_vec, int left_boundary, int right_boundary, string chrom, int adjust_base, bool debug)
	{
if(debug)
cout << "anchor mapping buffer size is " << _bufferedHits.size() << endl;
		if(_bufferedHits.size() > _n) 
		{
				_bufferedHits.resize(_n);
		}
		...
		{
if(debug)
cout << _bufferedHits[i].h.second << " " << adjust_base  << " " << (_bufferedHits[i].h.second + adjust_base) << " " << left_boundary << " " << right_boundary << endl;
			if((left_boundary >= 0 && (int)(_bufferedHits[i].h.second) + adjust_base < left_boundary) || (right_boundary >= 0 && (int)(_bufferedHits[i].h.second) + adjust_base > right_boundary))
				continue;
			Bwtmap_Info new_info;
			if(_bufferedHits[i].fw)
				new_info.strand = "+";
	...
	bool convert_repeat_segment(vector<Bwtmap_Info> & mapsplice_vec, bool paired, int _pair_no, int segment_no, string chrom, int adjust_base, int left_boundary, int right_boundary, bool debug)
	{
		if(debug)
		{
			if(_bufferedHits.size() > 0)
		...
		for(size_t i = 0; i < _bufferedHits.size(); i++)
		{
			...
			mapsplice_vec.push_back(new_info);
			if(debug)
				cout << " convert_repeat_segment " << new_info.read_id << "\t" << new_info.start_seg_no << "\t" << new_info.chrom << "\t" << new_info.strand << "\t" <<new_info.start << "\t" <<new_info.end << endl;
		}
		return true;
	}
	
	void compute_offset_jumpcode_old(int real_offset, char* real_jumpcode, int offset, int read_len, int& result_start, int& result_end, vector<Jump_Code>& result_jump_code)
	{
		const char * jumpcode_spliter = "MN"; 
		char *strtok_r_pointer;
		result_start = real_offset - 1;
		char* p;
		p = strtok_r(real_jumpcode ,jumpcode_spliter, &strtok_r_pointer); 
		int step=0;
		int match_len=0;
//cout << "original result start " << result_start << " offset is " << offset <<endl; 
		...
		{ 
			if(step%2 == 0)    //match
			{
				...
				if(match_len > offset)    //skipped enough
				{
					result_start += offset;
					match_len -= offset;
//cout << "skipped enough " << result_start << endl; 
					break;
				}
				else             // still need to skip
				{
					result_start += match_len;
					offset -= match_len;
//cout << "still need to skip " << result_start << endl; 
				}
			} 
			...
			step++;
			p = strtok_r(NULL , jumpcode_spliter, &strtok_r_pointer); 
		} 
		p = strtok_r(NULL , jumpcode_spliter, &strtok_r_pointer);     //get next N
		if(match_len>read_len)          //generate jump code
		{
			Jump_Code new_jump_code(read_len, "M");
			result_jump_code.push_back(new_jump_code);
		}
		else
		{
			Jump_Code new_jump_code(match_len, "M");
			result_jump_code.push_back(new_jump_code);
			read_len-=match_len;
			step=1;
			while(p != NULL)       
			{ 
				...
				{
					int jump_len=atoi(p);
					Jump_Code new_jump_code(jump_len, "N");
					result_jump_code.push_back(new_jump_code);
				}
				step++;
				p = strtok_r(NULL , jumpcode_spliter, &strtok_r_pointer); 
		...
		for(size_t i = 0; i < result_jump_code.size(); i++)
		{
			result_end += result_jump_code[i].len;
		}
	}
	
	void compute_offset_jumpcode(int real_offset, char* real_jumpcode, int offset, int read_len, int& result_start, int& result_end, vector<Jump_Code>& result_jump_code, bool head_0M, bool tail_0M)
	{
		const char * jumpcode_spliter = "MN"; 
		char *strtok_r_pointer;
		result_start = real_offset - 1;
...
//cout << "original result start " << result_start << " offset is " << offset <<endl; 
		...
		{ 
			...
			{
				match_len=atoi(p);
				if(match_len > offset || (head_0M && match_len == offset))    //skipped enough
				{
					result_start += offset;
					match_len -= offset;
//cout << "skipped enough " << result_start << endl; 
...
//cout << "still need to skip " << result_start << endl; 
		...
		if(match_len > read_len || (!tail_0M && match_len == read_len))          //generate jump code
		{
			Jump_Code new_jump_code(read_len, "M");
			result_jump_code.push_back(new_jump_code);
		}
		else
		{
			...
			{ 
				...
				{
					int match_len=atoi(p);
					if(match_len > read_len || (!tail_0M && match_len == read_len))
					{
						Jump_Code new_jump_code(read_len, "M");
						result_jump_code.push_back(new_jump_code);
						break;
	...
	bool convert_remap(size_t bwt_index1, size_t bwt_index2, int premapped_len, vector<Splice_Info>& fixed_splice_vec, string chrom_bound, int left_bound, int right_bound, int left_most, int right_most, vector<string>& remap_Refnames, bool head_0M, bool tail_0M, bool debug)
	{
		if(debug)
			cout << "converting remap" << endl;
		if(_bufferedHits.size() > _n) 
		{
				_bufferedHits.resize(_n);
		}
		bool fixed = false;
		for(size_t hit_index = 0; hit_index < _bufferedHits.size(); hit_index++)
		{
			size_t read_len = length(_bufferedHits[hit_index].patSeq);
			size_t chrom_len = remap_Refnames[_bufferedHits[hit_index].h.first].length();
			char* chromname = new char[chrom_len + 1];
			strcpy(chromname, remap_Refnames[_bufferedHits[hit_index].h.first].c_str());
			char real_chrom[1000], real_jumpcode[1000], real_offset_str[1000];
			int real_offset;
			char* p = chromname;
			int i = 0;
			int offset_pos = 0;
			...
			strncpy (real_chrom, p, offset_pos);
			real_chrom[offset_pos] = '\0';
			string chrom = real_chrom;
			strncpy (real_offset_str, (p + offset_pos + 1), jumpcode_pos - offset_pos - 1);
			real_offset_str[jumpcode_pos - offset_pos - 1] = '\0';
			char* pEnd;
			real_offset = strtol(real_offset_str, &pEnd, 10);
			strncpy (real_jumpcode, (p + jumpcode_pos + 1), i - jumpcode_pos - 1);
			real_jumpcode[i - jumpcode_pos - 1] = '\0';
			delete[] chromname; 
			if(chrom != chrom_bound)
				continue;	
			int remap_offset = _bufferedHits[hit_index].h.second;
      Splice_Info new_splice;
if(debug)
cout << "remapped to" << "\t" << remap_Refnames[_bufferedHits[hit_index].h.first] << "\t" << _bufferedHits[hit_index].h.second <<endl; 
			compute_offset_jumpcode(real_offset, real_jumpcode, remap_offset, read_len, new_splice.start_pos, new_splice.end_pos, new_splice.jump_code, head_0M, tail_0M); 
if(debug)
cout << left_most << "\t" << left_bound << "\t< " << new_splice.start_pos << "\t" << new_splice.end_pos << " >\t" << right_bound << "\t" << right_most << endl;
			if(new_splice.jump_code.size() == 0 || (left_most != -1 && new_splice.start_pos < left_most) || (right_most != -1 && new_splice.end_pos > right_most) || (left_bound != -1 && new_splice.start_pos != left_bound) || (right_bound != -1 && new_splice.end_pos != right_bound))
				continue;
			new_splice.start_contig = bwt_index1;
			new_splice.end_contig = bwt_index2;
			fixed_splice_vec.push_back(new_splice);
			fixed = true;
		}
		if(debug)
			cout << "converting remap complete, there is matched remap: " << fixed << endl;
		return fixed;	
	}
	
	int compute_offset_jumpcode_fusion_forward(int real_offset, char* real_jumpcode, int offset, int read_len, int& result_start, int& result_end, vector<Jump_Code>& result_jump_code, bool left_splice)
	{
		const char * jumpcode_spliter = "MN"; 
		char *strtok_r_pointer;
		result_start = real_offset - 1;
...
//cout << "still need to skip " << result_start << endl; 
		...
		if(left_splice && (offset >= 0 || match_len >= read_len))
			return -1;
		if(match_len >= read_len)          //generate jump code
		{
			Jump_Code new_jump_code(read_len, "M");
	...
	int compute_offset_jumpcode_fusion_reverse(int real_offset, char* real_jumpcode, int offset, int read_len, int& result_start, int& result_end, vector<Jump_Code>& result_jump_code, bool left_splice)
	{
		const char * jumpcode_spliter = "MN"; 
		char *strtok_r_pointer;
		result_end = real_offset - 1;
		char* p;
		p = strtok_r(real_jumpcode ,jumpcode_spliter, &strtok_r_pointer); 
		int step=0;
		int match_len=0;
...
//cout << "still need to skip " << result_start << endl; 
		...
		{
			Jump_Code new_jump_code(read_len, "M");
			result_jump_code.insert(result_jump_code.begin(), new_jump_code);
		}
		else
		{	
			Jump_Code new_jump_code(match_len, "M");
		...
		for(size_t i = 0; i < result_jump_code.size(); i++)
		{
			result_start -= result_jump_code[i].len;
		}
		return read_len;
	}
	
	bool convert_fusion_remap(size_t index1, size_t index2, int premapped_len, int buffer_len, vector<Fusion_Splice>& fixed_splice_vec, string chrom_bound1, string chrom_bound2, string strand_bound1, string strand_bound2, int left_bound, int right_bound, vector<string>& remap_Refnames, map<string, string>& reference_sequence, bool debug)
	{
		if(debug)
			cout << "converting fusion remap, buffer size is " << _bufferedHits.size() << " threshold is " << _n << endl;
		if(_bufferedHits.size() > _n) 
		{
				_bufferedHits.resize(_n);
		}
		...
		for(size_t hit_index = 0; hit_index < _bufferedHits.size(); hit_index++)
		{
			int read_len = (int)(length(_bufferedHits[hit_index].patSeq));
			size_t chrom_len = remap_Refnames[_bufferedHits[hit_index].h.first].length();
			char* chromname = new char[chrom_len + 1];
			if(debug)
				cout << "remap to " << remap_Refnames[_bufferedHits[hit_index].h.first] << (_bufferedHits[hit_index].fw ? " +" : " -") << " , offset is " << _bufferedHits[hit_index].h.second << endl;
			strcpy(chromname, remap_Refnames[_bufferedHits[hit_index].h.first].c_str());
			char real_chrom1[1000], real_chrom2[1000], real_jumpcode1[1000], real_jumpcode2[1000], real_offset_str1[1000], real_offset_str2[1000];
			char* p = chromname;
			int real_offset1;
			int real_offset2;
			int offset_pos1 = 0;
			...
			Splice_Info left_splice;
      Splice_Info right_splice;
			strncpy (real_chrom1, p, offset_pos1 - 2);
			real_chrom1[offset_pos1 - 2] = '\0';
			left_splice.chrom = real_chrom1;
			strncpy (real_chrom2, p + split_pos + 1, offset_pos2 - split_pos - 3);
			real_chrom2[offset_pos2 - split_pos - 3] = '\0';
			right_splice.chrom = real_chrom2;
			char* pEnd1;
			char* pEnd2;
			strncpy (real_offset_str1, (p + offset_pos1 + 1), jumpcode_pos1 - offset_pos1 - 1);
			real_offset_str1[jumpcode_pos1 - offset_pos1 - 1] = '\0';
			real_offset1 = strtol(real_offset_str1, &pEnd1, 10);
			strncpy (real_offset_str2, (p + offset_pos2 + 1), jumpcode_pos2 - offset_pos2 - 1);
			real_offset_str2[jumpcode_pos2 - offset_pos2 - 1] = '\0';
			real_offset2 = strtol(real_offset_str2, &pEnd2, 10);
			strncpy (real_jumpcode1, (p + jumpcode_pos1 + 1), split_pos - jumpcode_pos1 - 1);
			real_jumpcode1[split_pos - jumpcode_pos1 - 1] = '\0';
			strncpy (real_jumpcode2, (p + jumpcode_pos2 + 1), i - jumpcode_pos2 - 1);
			real_jumpcode2[i - jumpcode_pos2 - 1] = '\0';
			char fusion_junc_start_strand = *(p + offset_pos1 - 1);
			char fusion_junc_end_strand = *(p + offset_pos2 - 1);
			left_splice.strand = (_bufferedHits[hit_index].fw == (fusion_junc_start_strand =='+')) ? "+" : "-";
			right_splice.strand = (_bufferedHits[hit_index].fw == (fusion_junc_end_strand =='+')) ? "+" : "-";
			delete[] chromname; 
		 	Splice_Info* first_splice;
      Splice_Info* second_splice;
		  if(_bufferedHits[hit_index].fw)
		  {
		  	first_splice = &left_splice;
				second_splice = &right_splice;
		  }
		  else
		  {
		  	first_splice = &right_splice;
				second_splice = &left_splice;
		  }
		  if(debug)
		  {
		  	cout << "convert fusion, bound\t" << chrom_bound1 << "\t" << chrom_bound2 << "\t" << strand_bound1 << "\t" << strand_bound2 <<endl;
		  	cout << "convert fusion, real\t" << first_splice->chrom << "\t" << second_splice->chrom << "\t" << first_splice->strand << "\t" << second_splice->strand <<endl;
		  }
		  if((chrom_bound1 != "*" && first_splice->chrom != chrom_bound1) || (chrom_bound2 != "*" && second_splice->chrom != chrom_bound2) || (strand_bound1!= "*" && first_splice->strand != strand_bound1) || (strand_bound2 != "*" && second_splice->strand != strand_bound2))
		  	continue;
			int remap_offset = _bufferedHits[hit_index].h.second;
			if(fusion_junc_start_strand =='+')
				read_len = compute_offset_jumpcode_fusion_forward(real_offset1, real_jumpcode1, remap_offset, read_len, left_splice.start_pos, left_splice.end_pos, left_splice.jump_code, true); 
			else
				read_len = compute_offset_jumpcode_fusion_reverse(real_offset1, real_jumpcode1, remap_offset, read_len, left_splice.start_pos, left_splice.end_pos, left_splice.jump_code, true); 			
			if(read_len == -1)
				continue;
			if(fusion_junc_end_strand =='+')
				read_len = compute_offset_jumpcode_fusion_forward(real_offset2, real_jumpcode2, 0, read_len, right_splice.start_pos, right_splice.end_pos, right_splice.jump_code, false); 
			else
				read_len = compute_offset_jumpcode_fusion_reverse(real_offset2, real_jumpcode2, 0, read_len, right_splice.start_pos, right_splice.end_pos, right_splice.jump_code, false); 
			if(debug)
		  {
		  	cout << "convert fusion, bound\t" << left_bound << "\t" << right_bound <<endl;
		  	cout << "convert fusion, real\t" << first_splice->start_pos << "\t" << first_splice->end_pos << "\t" <<  second_splice->start_pos << "\t" <<  second_splice->end_pos <<endl;
		  }
			if((strand_bound1 == "+" && left_bound != -1 && first_splice->start_pos != left_bound) || (strand_bound1 == "-" && left_bound != -1 && first_splice->end_pos != left_bound) || (strand_bound2 == "+" && right_bound != -1 && second_splice->end_pos != right_bound) || (strand_bound2 == "-" && right_bound != -1 && second_splice->start_pos != right_bound))
				continue;
			first_splice->Get_Extra_Info(reference_sequence[first_splice->chrom]);
			second_splice->Get_Extra_Info(reference_sequence[second_splice->chrom]);	
			read_len = (int)(length(_bufferedHits[hit_index].patSeq));
			//if(debug)
		  // cout << "read len is\t" << read_len << "\tleft and right mismatch size\t" << left_splice.mismatches.size() << "\t" << right_splice.mismatches.size() << endl;
			string flank_seq;
			if(first_splice->strand == "+")
				flank_seq += reference_sequence[first_splice->chrom].substr(first_splice->end_pos + 1, 2);
			else
				flank_seq += revcomp(reference_sequence[first_splice->chrom].substr(first_splice->start_pos - 2, 2));
			if(second_splice->strand == "+")
				flank_seq += reference_sequence[second_splice->chrom].substr(second_splice->start_pos - 2, 2);
			else
				flank_seq += revcomp(reference_sequence[second_splice->chrom].substr(second_splice->end_pos + 1, 2));
			first_splice->start_contig = index1;
			first_splice->end_contig = index2;
			first_splice->buffer_len = buffer_len;
			second_splice->buffer_len = buffer_len;
			if(debug)
			{
				cout << "first splice jump code: ";
				for(size_t x = 0; x < first_splice->jump_code.size(); x++)
				{
					cout << first_splice->jump_code[x].toString(); 
				}
				cout << ", second splice jump code: ";
				for(size_t x = 0; x < second_splice->jump_code.size(); x++)
				{
					cout << second_splice->jump_code[x].toString(); 
				}
				cout << endl;
			}
			Fusion_Splice new_fusion_splice(*first_splice, *second_splice, flank_seq);
			fixed_splice_vec.push_back(new_fusion_splice);
			fixed = true;
		}
		if(debug)
			cout << "converting remap complete, there is matched remap: " << fixed << endl;
	...
	void clear_bufferedHits()
	{
			_bufferedHits.clear();
	}
	
	vector<Hit>& get_bufferedHits()
	{
		return _bufferedHits;
	}
	...
	void set_max_hits(uint32_t new_max)
	{
		_max = new_max;
		_n = new_max;
	}
