...
#define RESET_BUF_LEN(str, buf, len, typ) _setBegin(str, (typ*)buf); _setLength(str, len); _setCapacity(str, BUF_SIZE);
	...
	void copy(ReadBuf& _readbuf){
		patid = _readbuf.patid;
		mate= _readbuf.mate;
		readOrigBufLen = _readbuf.readOrigBufLen;
		qualOrigBufLen = _readbuf.qualOrigBufLen;
		alts = _readbuf.alts;
		trimmed5 = _readbuf.trimmed5;
		trimmed3 = _readbuf.trimmed3;
		fuzzy = _readbuf.fuzzy;
		color = _readbuf.color;
		primer = _readbuf.primer;
		trimc = _readbuf.trimc;
		seed = _readbuf.seed;
		
		name = _readbuf.name;
		patFw= _readbuf.patFw;
		patRc = _readbuf.patRc;
		qual = _readbuf.qual;
		patFwRev = _readbuf.patFwRev;
		patRcRev = _readbuf.patRcRev;
		qualRev = _readbuf.qualRev;

		for(int i=0; i<3; i++)
		{
			altPatFw[i] = _readbuf.altPatFw[i];
			altPatRc[i] = _readbuf.altPatRc[i];
			altQual[i] = _readbuf.altQual[i];
			altPatFwRev[i] = _readbuf.altPatFwRev[i];
			altPatRcRev[i] = _readbuf.altPatRcRev[i];
			altQualRev[i] = _readbuf.altQualRev[i];
		}
		memcpy(nameBuf, _readbuf.nameBuf, sizeof(_readbuf.nameBuf));
		memcpy(patBufFw, _readbuf.patBufFw, sizeof(_readbuf.patBufFw));
		memcpy(patBufRc, _readbuf.patBufRc, sizeof(_readbuf.patBufRc));
		memcpy(qualBuf, _readbuf.qualBuf, sizeof(_readbuf.qualBuf));
		memcpy(patBufFwRev, _readbuf.patBufFwRev, sizeof(_readbuf.patBufFwRev));
		memcpy(patBufRcRev, _readbuf.patBufRcRev, sizeof(_readbuf.patBufRcRev));
		memcpy(qualBufRev, _readbuf.qualBufRev, sizeof(_readbuf.qualBufRev));

		memcpy(altPatBufFw, _readbuf.altPatBufFw, sizeof(_readbuf.altPatBufFw));
		memcpy(altPatBufRc, _readbuf.altPatBufRc, sizeof(_readbuf.altPatBufRc));
		memcpy(altQualBuf, _readbuf.altQualBuf, sizeof(_readbuf.altQualBuf));
		memcpy(altPatBufFwRev, _readbuf.altPatBufFwRev, sizeof(_readbuf.altPatBufFwRev));
		memcpy(altPatBufRcRev, _readbuf.altPatBufRcRev, sizeof(_readbuf.altPatBufRcRev));
		memcpy(altQualBufRev, _readbuf.altQualBufRev, sizeof(_readbuf.altQualBufRev));
	}
	
	void getSegmentOf(ReadBuf& fullRead, int start, int end, uint32_t seed_, int seg_no)
	{
		reset();
		for(int i=start; i <= end; i++)
		{
			appendValue(patFw, fullRead.patFw[i]);
			patBufFw[i] = fullRead.patBufFw[i];
			appendValue(qual, fullRead.qual[i]);
			qualBuf[i] = fullRead.qualBuf[i];
		}
		name=fullRead.name;  
		append(name, "~"); 
		append(name, int_to_str(seg_no));
		//appendValue(name, (char)(seg_no + 48));    
		patid = fullRead.patid * 10 + seg_no;
		mate = fullRead.mate;    
		alts = fullRead.alts;
		trimmed5 = fullRead.trimmed5;
		trimmed3 = fullRead.trimmed3;
		fuzzy = fullRead.fuzzy;
		color = fullRead.color;
		primer = fullRead.primer;
		trimc = fullRead.trimc;         
		constructRevComps();
		constructReverses();
		seed = genRandSeed(patFw, qual, name, seed_);
	}
	
	void getAnchorOf(ReadBuf& fullRead, int start, int end, uint32_t seed_, size_t anchor_id){
		reset();
		for(int i = start; i <= end; i++)
		{
			appendValue(patFw, fullRead.patFw[i]);
		...
		name=fullRead.name;  
		string tmp_string = "_anchor";
		append(name, tmp_string); 
		append(name, int_to_str((int)anchor_id));    
		patid = fullRead.patid * 10 + anchor_id;
		mate = fullRead.mate;    
		alts = fullRead.alts;
		trimmed5 = fullRead.trimmed5;
		trimmed3 = fullRead.trimmed3;
		...
		void getRemapOf(ReadBuf& fullRead, int start, int end, uint32_t seed_, size_t anchor_id){
		reset();
		for(int i = start; i <= end; i++)
		{
			appendValue(patFw, fullRead.patFw[i]);
	...
	void assign_readid_to_read_block(Read_Block& read_block)
	{
		/*if(read_block.paired)
		{
			string full_read_id;
			assign(full_read_id, name);
			char read_id_tmp[1000];
			sscanf(full_read_id.c_str(), "%s", read_id_tmp);	
			read_block.read_id = read_id_tmp;
		}
		else*/
			assign(read_block.read_id, name);
	}
		
	void add_to_read_block(Read_Block& read_block)
	{
		string new_seq, new_seq_revcom, new_qual, new_qual_rev;
		assign(new_seq, patFw);
		assign(new_seq_revcom, patRc);
		assign(new_qual, qual);
		assign(new_qual_rev, qualRev);
		read_block.read_seq.push_back(new_seq);
		read_block.read_seq_revcom.push_back(new_seq_revcom);
		read_block.read_quality.push_back(new_qual);
		read_block.read_quality_revcom.push_back(new_qual_rev);
	}
...
public:
	...
	void set_readbufa(ReadBuf& _read_buf)
	{
		buf1_.copy(_read_buf);
	}
	
	void set_readbufa_segment(ReadBuf& _read_buf, int start, int end, uint32_t _seed, size_t seg_no)
	{
		buf1_.getSegmentOf(_read_buf, start, end, _seed, seg_no);
	}
	
	void set_readbufa_anchor(ReadBuf& _read_buf, int start, int end, uint32_t _seed, size_t anchor_id)
	{
		buf1_.getAnchorOf(_read_buf, start, end, _seed, anchor_id);
	}
	
	void set_readbufa_remap(ReadBuf& _read_buf, int start, int end, uint32_t _seed, size_t anchor_id)
	{
		buf1_.getRemapOf(_read_buf, start, end, _seed, anchor_id);
	}
