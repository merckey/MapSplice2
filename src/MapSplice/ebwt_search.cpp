...
static struct option long_options[] = {
	...
	{(char*)"min_fusion_distance",            	   required_argument,       0,            ARG_MIN_FUSION_DIST},
...
static void parseOptions(int argc, const char **argv) {
	...
	do {
		...
		switch (next_option) {
			...
			case ARG_ORIG:
				...
				origString = optarg;
				break;
			case ARG_MIN_LEN: minimum_read_length = parseInt(16, "--min_len shortest read that will be aligned must be at least 16");  break;	
			case ARG_SEG_LEN: segment_length = parseInt(16, "--seg_len segment length must be at least 16");  break;	
			...
			case ARG_MIN_INTRON: min_intron = parseInt(1, "--min_intron minimum intron must be at least 1"); break;
			case ARG_MAX_INTRON_DOUBLE: max_intron_double_anchor = parseInt(1, "--max_intron_double maximum intron must be at least 1"); break;
			case ARG_MAX_INTRON_SINGLE: max_intron_single_anchor = parseInt(1, "--max_intron_single maximum intron must be at least 1"); break;
			...
			case ARG_MIN_FUSION_DIST: min_fusion_distance = parseInt(0, "--min_fusion_distance must be at least 0"); break;
			case ARG_QUAL_SCALE: 
			{
	...
	if(min_intron <= max_deletion) {
		cerr << "Error: " << " --min_intron minimum intron must be larger than max deletion"  << endl;
		throw 1;
	}
	if(max_intron_double_anchor <= min_intron) {
		cerr << "Error: " << " --max_intron_double maximum intron must be larger than minimum intron"  << endl;
		throw 1;
	}
	if(max_intron_single_anchor <= min_intron) {
		cerr << "Error: " << " --max_intron_single maximum intron must be larger than minimum intron"  << endl;
		throw 1;
	}
