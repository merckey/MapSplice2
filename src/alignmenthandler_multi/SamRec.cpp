...
void SamRec::Set(const string& line/*, int min_ins*/) 
{
	...
	if (is_fusion)
	{
		...
		//if (true || string(chrom) < string(chrom2) || (string(chrom) == string(chrom2) && start <= start2))
		{
		...
		if (is_swapped)
		{
			string mapseq1 = mapped_seq.substr(0, mappedlen2 - insertlen2);

			string mapseq2 = mapped_seq.substr(mapped_seq.length() - mappedlen1 + insertlen1, mappedlen1 - insertlen1);

			mapped_seq = mapseq2 + mapseq1;

			string qualstr1 = qual_str.substr(0, mappedlen2 - insertlen2);

			string qualstr2 = qual_str.substr(qual_str.length() - mappedlen1 + insertlen1, mappedlen1 - insertlen1);
	...
	else //normal alignment
	{
		...
		if ((splice_way.find("S") != string::npos))
		{
			...
			if (splice_way[splice_way.length() - 1] == 'S')
			{
				...
				tail_soft_clip = (size_t)(atoi(splice_way.substr(m_idx + 1, splice_way.length() - m_idx - 1).c_str()));
			...
			if (s_index != string::npos)
			{
				head_soft_clip = (size_t)(atoi(splice_way.substr(0, s_index).c_str()));
...
SamRec::tostring(size_t total, size_t idx) const
{
	...
	else
	{
		...
		{
			sprintf(sam_rec_char, "%s\t%hu\t%s\t%llu\t%hu\t%s\t%s\t%llu\t%d\t%s\t%s\tNM:i:%hu\tIH:i:%llu\tHI:i:%llu\tXS:A:%c\tXF:Z:%s"/*\t%s",*//*\t%d\t%lf"*/, tag_name.c_str(), strand_t, 
				chrom_name.c_str(), start, confid, ori_splice_way.c_str(), mate_match.c_str(), mate_offset, mate_diff, mapped_seq.c_str(), qual_str.c_str(), mis_match, total, idx, xs_tag, toflankstring().c_str()/*, clippled_way.c_str()*//*, best, filter_score*/);
		}
	}
...
string
SamRec::tostandfusion(size_t total, size_t idx) const 
{
	char sam_rec_char[5000];
	...
	else if (is_fusion)
	{
		...
		sprintf(sam_rec_char, "%s\t%hu\t%s\t%llu\t255\t%s\t%s\t%llu\t%ld\t%s\t%s\tNM:i:%hu\tIH:i:%llu\tHI:i:%llu\tYI:i:%llu\tYH:Z:%llu.1\tXS:A:%c\tXF:Z:%s\tZF:Z:FUS_%llu_%llu(%c%c)\n%s\t%hu\t%s\t%llu\t255\t%s\t%s\t%llu\t%ld\t%s\t%s\tNM:i:%hu\tIH:i:%llu\tHI:i:%llu\tYI:i:%llu\tYH:Z:%llu.2\tXS:A:%c\tXF:Z:%s\tZF:Z:FUS_%llu_%llu(%c%c)", 
			tag_name.c_str(), strand_t, chrom_name.c_str(), start, sp1.c_str(), 
			mate_match.c_str(), mate_offset, mate_diff, mapped_seq.c_str(), qual_str.c_str(), mis_match, total * 2, idx * 2 - 1, total, idx, xs_tag, combflankstring.c_str(), fusion_prefix_end, fusion_suffix_st, strand1, strand2,
			tag_name.c_str(), strand_t2, chrom_name2.c_str(), start2, sp2.c_str(), 
			mate_match2.c_str(), mate_offset2, mate_diff2, mapped_seq.c_str(), qual_str.c_str(), mis_match, total * 2, idx * 2, total, idx, xs_tag, combflankstring.c_str(), fusion_prefix_end, fusion_suffix_st, strand1, strand2);
	}
	else
		return "";
...
cout << "clipping by small anchor " << endl;
cout << "add_s " << add_S << endl;
...
{
	//need to recover original read sequence of trimmed part
...
SamRec::setfusionbit()
{
	...
	strand_t |=  (end_id == 1 ? IS_FIRST_END : IS_SECOND_END);
	...
	strand_t2 |= (end_id == 1 ? IS_FIRST_END : IS_SECOND_END);
}
...
bool SamRec::Swap()
{
	...
	string mapseq1 = mapped_seq.substr(0, mappedlen1 - insertlen1);

	string mapseq2 = mapped_seq.substr(mapped_seq.length() - mappedlen2 + insertlen2, mappedlen2 - insertlen2);

	mapped_seq = mapseq2 + mapseq1;

	string qualstr1 = qual_str.substr(0, mappedlen1 - insertlen1);

	string qualstr2 = qual_str.substr(qual_str.length() - mappedlen2 + insertlen2, mappedlen2 - insertlen2);
